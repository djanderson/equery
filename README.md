...
* In a similar vein, _dol-sen@freenode_ prodded and helped with a complete rewrite of the dependency handling stuff in *equery*. This task is about 80 percent finished, and we've seen an awesome increase in usability for external consumers, but more importantly, speed! Check this out:
```bash
...
```
(Above commit message inlined for posterity's sake)
>Lots of mucking around:
>* Added vim modelines to most files I touched. vim users, just type :set
>  modeline in a gentoolkit file to get set up quickly.
>* Started changing a lot about exception handling throughout gentoolkit.
>  Basically, we try to let all "for import" modules raise all exceptions, and
>  all exceptions are more normal -- they have an attribute with the offending
>  argument and a __str__ method that returns a pretty err message. We try not
>  to catch any fatal exceptions inside these modules and let the importing
>  script handle them now. This makes it a LOT nicer to work with as a library.
>* I've taken print_{error,info,warn} out of pprinter and added pp.error and
>  pp.warn. These two new fuctions return formatted strings instead of printing
>  them directly to the screen, so instead of pp.print_error('str') you should
>  do sys.stderr.write(pp.error('str')). This makes it easier for importing
>  scripts to do what they want with the error messages (log them and discard
>  the output, for example). Again, more proper behavior for a library.
>  pp.die still prints to stderr and exits.
>* depends.py has morphed again. A few commits ago I enabled finding depends for
>  masked packages. Looking more closely at this, I realize this feature has
>  never been enabled and really only complicates things. In fact we've been way
>  over complicating things by letting the user's query match more than one
>  package and then trying to find all the deps for those matches (this is also
>  how the original equery depends behaved, just not for masked). This is really
>  useless behavior. Imagine that 'equery d gcc' matches 4 or 5 versions of gcc,
>  then equery goes about finding packages that depend of any of those versions.
>  equery then lumps them all together without specifying which pkg depends on
>  which versin of gcc. In fact, the user isn't made aware that more than one
>  version of their query is being matched against. In this commit, depends uses
>  portage's "best-match" capability to find a single unmasked package to find
>  deps for. It's dead simple and fast, and it makes more sense.
>* depends.py still doesn't support indirect searching or --show-progress (for
>  dol-sen) but that patch will be worked in again ASAP.
